...
- [ ] The validation workflow passes on Python 3.10–3.13 after push.
...
- [ ] `v1.0.0` matches every manifest and the changelog.

## GitHub launch

- [ ] Approve the English and Italian founder story in `submission/launch-copy.md`.
- [ ] Decide whether to rename the remote repository to `agent-fieldbook`; GitHub redirects preserve the old URL.
- [ ] Set the description to “Source-backed skills that turn official Claude and Codex training into reliable daily workflows.”
- [ ] Add topics: `agent-skills`, `claude`, `codex`, `prompt-engineering`, `subagents`, `ai-fluency`.
...
- [ ] Push the reviewed changes, create tag `v1.0.0`, and verify the release workflow artifacts.
...
External submission, account verification, repository renaming, pushing, tagging,
and publication require the maintainer's authenticated action and are not
